  ...
  if (question.type === 'text') {
    return (
      <textarea
        ...
        rows={3}
        placeholder={t('answer_optional_placeholder')}
        className="w-full rounded-2xl px-5 py-4 text-[14px] focus:outline-none resize-none"
        style={{ background: 'var(--ivory)', border: '1px solid var(--line)', color: 'var(--ink)', fontFamily: 'inherit' }}
      />
    );
  }
  ...
  if (question.type === 'rating') {
    ...
    return (
      <div className="flex items-center gap-2 flex-wrap">
        {[1, 2, 3, 4, 5].map((n) => {
          ...
          return (
            <button
              ...
              style={{
                width: 48, height: 48, borderRadius: 16, fontSize: 18, fontWeight: 800,
        ...
        {value != null && (
          <span className="ml-2 text-[16px]" style={{ color: 'var(--ink)', fontWeight: 800 }}>
        ...
        {(question.options ?? []).map((option) => {
          ...
          return (
            <label
              ...
              style={{ background: active ? 'var(--green-soft)' : '#ffffff', border: `1px solid ${active ? 'var(--green)' : 'var(--line)'}` }}
            >
              <input type="radio" name={question.id} value={option} checked={active} onChange={() => onChange(option)} className="sr-only" />
              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ border: `2px solid ${active ? 'var(--green)' : 'var(--line)'}` }}>
                {active && <div className="w-2.5 h-2.5 rounded-full" style={{ background: 'var(--green)' }} />}
              </div>
              <span className="text-[14px]" style={{ color: 'var(--ink)', fontWeight: 500 }}>{option}</span>
            </label>
          );
        })}
      </div>
        ...
        {(question.options ?? []).map((option) => {
          ...
          return (
            <label
              key={option}
              className="flex items-center gap-3 cursor-pointer rounded-2xl px-5 py-4 transition-all"
              style={{ background: checked ? 'var(--green-soft)' : '#ffffff', border: `1px solid ${checked ? 'var(--green)' : 'var(--line)'}` }}
            >
              <input
                type="checkbox"
                checked={checked}
                ...
                className="sr-only"
              />
              <div className="w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0" style={{ background: checked ? 'var(--green)' : 'transparent', border: `2px solid ${checked ? 'var(--green)' : 'var(--line)'}` }}>
                {checked && (
...
function SurveyForm({ survey, onBack }: { survey: Survey; onBack: () => void }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (missing.length > 0) { setError(t('error_mandatory')); return; }
    setLoading(true);
    try {
      ...
      setError(status === 409 ? t('already_responded') : t('error_generic'));
    } finally {
      setLoading(false);
    }
  }
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center px-6 pt-24 text-center fade-up">
        ...
        <button onClick={onBack} className="text-[13px] underline" style={{ color: 'var(--green)', fontWeight: 700 }}>
          {t('see_other_surveys')}
        </button>
      </div>
    );
  ...
  return (
    <div className="fade-up">
      <div className="px-6 pt-6 pb-6">
        <button onClick={onBack} className="flex items-center gap-1.5 text-[12px] mb-4" style={{ color: 'var(--ink-faint)', fontWeight: 500 }}>
          <ArrowLeft size={13} /> {t('back')}
        </button>
        <p className="text-[10px] uppercase tracking-[0.05em] mb-3" style={{ color: 'var(--green)', fontWeight: 700 }}>
          {t('survey_label')}
        </p>
        <h2 className="text-titre" style={{ color: 'var(--ink)' }}>{survey.title}</h2>
        <p className="text-[12px] mt-2" style={{ color: 'var(--ink-faint)' }}>
          {t('questions', survey.questions.length)}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="px-6 pb-6 space-y-8">
        {survey.questions.map((q, i) => (
          <section key={q.id}>
            <p className="text-[11px] uppercase tracking-[0.05em] mb-2" style={{ color: 'var(--ink-faint)', fontWeight: 700 }}>
              {t('question_label')} {i + 1}
              {q.type !== 'text' && <span style={{ color: 'var(--green)', marginLeft: 4 }}>·</span>}
            </p>
            <p className="text-[18px] mb-4" style={{ color: 'var(--ink)', fontWeight: 800, lineHeight: 1.3 }}>
              {q.label}
            </p>
            <QuestionField
              question={q}
        ...
        <div className="sticky bottom-0 z-10 -mx-6 px-6 pt-10 pb-5" style={{ background: 'linear-gradient(to bottom, transparent, var(--cream) 45%)' }}>
          ...
          >
            {loading ? t('sending') : t('send_answers')}
          </button>
        </div>
      </form>
    </div>
...
export function SondagesPage() {
  ...
  return (
    <AppLayout title={t('surveys_title')}>
      {selected ? (
        ...
        <div className="fade-up">
          ...
          <div className="px-6 pb-10">
            {loading ? (
              ...
              <div className="rounded-3xl py-16 text-center" style={{ background: '#ffffff', border: '1px solid var(--line)' }}>
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: 'var(--green-soft)' }}>
                  <ClipboardList size={20} style={{ color: 'var(--green)' }} />
                </div>
                <p className="text-[18px]" style={{ color: 'var(--ink)', fontWeight: 800 }}>{t('no_surveys')}</p>
                <p className="text-[12px] mt-1" style={{ color: 'var(--ink-faint)' }}>{t('come_back_soon')}</p>
              </div>
            ) : (
                ...
                {surveys.map((survey) => (
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-[17px] leading-tight" style={{ color: 'var(--ink)', fontWeight: 800 }}>
                        {survey.title}
                      </p>
                    ...
                    <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'var(--green)' }}>
