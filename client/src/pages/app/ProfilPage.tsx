...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ width: 46, height: 28, borderRadius: 999, background: checked ? 'var(--green)' : 'var(--line)', border: 'none' }}
    ...
    <AppLayout title={t('my_profile')}>
      <div className="px-6 pt-10 pb-8 flex flex-col items-center text-center fade-up">
        <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-4" style={{ background: 'var(--green)', color: '#ffffff' }}>
          <span className="text-[30px]" style={{ fontWeight: 800 }}>{initial}</span>
        </div>
        <p className="text-[10px] uppercase tracking-[0.05em] mb-2" style={{ color: 'var(--green)', fontWeight: 700 }}>
          {t('member')}
        </p>
        <p className="text-[20px]" style={{ color: 'var(--ink)', fontWeight: 800 }}>
      ...
      <div className="px-6 space-y-3">
        <div className="rounded-2xl p-5 flex items-center gap-4" style={{ background: '#ffffff', border: '1px solid var(--line)' }}>
      ...
      <div className="sticky bottom-0 z-10 px-6 pt-10 pb-6 mt-6" style={{ background: 'linear-gradient(to bottom, transparent, var(--cream) 45%)' }}>
        {error && (
        ...
        {saved && (
          <div className="text-[13px] rounded-xl px-4 py-3 mb-3 flex items-center gap-2 justify-center" style={{ background: 'var(--green-soft)', color: 'var(--green)', fontWeight: 700 }}>
            <Check size={14} /> {t('preferences_saved')}
          </div>
        )}
        <button
          onClick={handleSave}
